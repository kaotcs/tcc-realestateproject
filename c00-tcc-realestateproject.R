...
pacotes <- c('tidyverse', 'data.table', 'gridExtra', 'pixiedust', 'kableExtra', 'PerformanceAnalytics',
             'fastDummies', 'nortest', 'car', 'caret', 'plotly', 'jtools', 'Metrics', 'sjPlot')
...
ggplot(aux_ad, aes(x=ape_dorm)) + 
  geom_line(aes(y = med_sz, group=1, color = "Sem ZEU"), size=1.5) + 
  geom_line(aes(y = med_cz, group=2, color="Com ZEU"), linetype="twodash", size=1.5) +
...
yhat_ <- (((yhat_ * lambda_BC$lambda) + 1)) ^ (1 / lambda_BC$lambda)
